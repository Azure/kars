...
from rollout_diagnostics import collect as rollout_diagnostics
        ...
        try:
            ...
            if setup:
                ...
                if name == "private-bff-observer-and-fresh-privacy-rpc":
                    report["cases"][name]["routerRollout"] = rollout_diagnostics(
            ...
            print(json.dumps({"nativeCase": name, **{key: value for key, value in report["cases"][name].items()
                                                   if key not in ("metadataAtFailure", "observationReadiness",
                                                                  ...
                                                                  "enrollmentTemplateDrift", "rotationFailureSnapshot",
                                                                  "routerRollout")}}), flush=True)
