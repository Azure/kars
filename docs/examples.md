...
|---|---|---|---|
| [`basic-agent`](https://github.com/Azure/kars/tree/main/examples/basic-agent) | `OpenClaw` | YAMLs apply (dry-run + AKS); runtime path exercised by exec-brief harness | The smallest possible deployment — minimal sandbox with default isolation (seccomp-strict, RO rootfs, egress guard, AGT governance). **Start here.** |
| [`telegram-agent`](https://github.com/Azure/kars/tree/main/examples/telegram-agent) | `OpenClaw` | YAMLs apply; requires real Telegram bot token to exercise channel | OpenClaw agent wired to a Telegram channel via the [channel-plugin pattern](channels-plugins.md). |
| [`confidential-agent`](https://github.com/Azure/kars/tree/main/examples/confidential-agent) | `OpenClaw` + Kata VM isolation | ✅ Verified live on AKS with a `katapool` Kata-enabled nodepool (`runtimeClassName: kata-vm-isolation`, pod 2/2 Running). Won't schedule on plain `kind` clusters that lack the `kata-vm-isolation` RuntimeClass — that's expected. | The basic-agent shape upgraded to Kata VM isolation — dedicated kernel per pod, container-escape attacks trapped inside the VM. |
...
- [`examples/README.md`](https://github.com/Azure/kars/blob/main/examples/README.md) — the same catalogue, in-repo
