...
|---|---|---|
| PR / push to `dev` / `main` | `Chaos Tier` (CI) | `cargo test --package kars-chaos-tests --features chaos --tests --no-fail-fast` |
...
invocation is scoped to the `kars-chaos-tests` crate so it does
