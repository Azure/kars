...
flowchart TB
  subgraph AirGap["🔒 Air-gapped network island"]
    ...
    subgraph AKS["☸️ Local AKS / Azure Stack HCI / k3s"]
      ...
      subgraph SBX["📦 KarsSandbox 'analyst'"]
        OC["openclaw"]
...
sequenceDiagram
    ...
    Air->>Cls: helm install kars \<br/>--values offline-values.yaml \<br/>--set router.model.localUrl=https://model-svc
    Cls->>Cls: KarsSandbox spawn,<br/>NetworkPolicy enforces<br/>internal-only egress
```
...
| CRD | Air-gap role |
|---|---|
| `InferencePolicy` | Token budget (daily/monthly) + content safety against the local model. Referenced by `KarsSandbox.spec.inferenceRef.name` (omitting it → `Degraded/InferencePolicyNotFound`). |
...
| `KarsMemory` | Bind to an on-prem Foundry-Edge or compatible memory-store endpoint. |
| `KarsEval` | Schedule regression evals against your local model server. |
...
spec:
  ...
  networkPolicy:
    allowlistRef:                # signed OCI artifact bundled offline (see below)
      ...
      repository: kars-policy/analyst-egress
      digest: sha256:…
...
kars egress sign \
  ...
  --push private-registry.local/kars-policy/analyst-egress \
  --mode kms \
  --kms-key https://myvault.vault.azure.net/keys/airgap-signer/v1

# 2. Save the artifact + signature to the bundle tarball:
cosign save private-registry.local/kars-policy/analyst-egress \
...
cosign load \
  --dir bundle/policy/analyst-egress/ \
  private-registry.local/kars-policy/analyst-egress
...
docker push private-registry.local/kars/{controller,router,sandbox}:latest

helm install kars deploy/helm/kars \
  --values offline-values.yaml \
  --set image.repository=private-registry.local/kars \
...
kars add analyst --model llama-3.1-70b --governance \
...
- **BYO runtime or existing agent code.** Use `spec.runtime.kind: BYO` for a custom container declaring the `org.kars.runtime.contract` OCI label. Teams with existing Python OpenAI Agents or MAF code can use the first-class adapters (`OpenAIAgents`, `MicrosoftAgentFramework`) without changes to the governance or audit chain.
...
- `deploy/helm/kars/values.yaml` (`audit.sink`, `router.model.provider`, `signerPolicy.*`)
