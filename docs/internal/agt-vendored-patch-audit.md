...
| 2026-05-08 | TBD | v0.1.2 | v0.3.0 | v0.3.0 | Copilot <223556219+Copilot@users.noreply.github.com>, Pal Lakatos-Toth <pallakatos@microsoft.com> | dev → main integration (PR #242). `vendor/agentmesh-sdk/dist/` overlay refreshed to keep patches #5 (KNOCK race), #7, #8, and #12 in sync with dist artifacts touched during the GitHub Copilot provider work. Source-side TypeScript at `vendor/agentmesh-sdk/src/{dht/index,encryption/{prekey,ratchet,session,x3dh}}.ts` re-validated against patch documentation in `vendor/agentmesh-sdk/README.md`; no semantic delta vs the prior re-audit. SDK pin (v0.1.2), relay pin (v0.3.0), and registry pin (v0.3.0) unchanged. All 12 SDK patches re-verified still required against upstream v0.1.2 — none upstream-absorbed. |
