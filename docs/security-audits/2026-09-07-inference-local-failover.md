...
- Host matching uses parsed hostnames; ambient WI/IMDS tokens are refused for
  unrecognized non-Azure hosts. Dedicated local inference services receive no
  credentials, including when a provider credential is configured.
...
- Retry-worthy transport/429/5xx failures can use another configured provider.
- Ordinary auth/client errors are not failover triggers.
- Unavailable-model recovery is bounded to the configured default and cached
  per endpoint/model; Responses-only recovery remains with the actual selected
  provider and deployment.
- A successful stream is never replayed after a later body failure.
