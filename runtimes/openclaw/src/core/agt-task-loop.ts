...
export async function processTaskWithTools(
  ...
  log.info(`AGT task-loop: ${tools.length} tools loaded (strict=${strictCount}, AZURECLAW_STRICT_TOOLS=${process.env.AZURECLAW_STRICT_TOOLS || "<unset>"}, model=${process.env.OPENCLAW_MODEL || process.env.AZURECLAW_MODEL || "<unset>"})`);
          ...
          if (fnName === "file_write") {
            ...
            if (isRootNoParent) {
              ...
              result = JSON.stringify({
                error: "no_parent",
                message: "This agent IS the root coordinator — it has no parent to send to. The user dispatched the task directly. Reply with your final answer instead of routing it via 'parent'. If you need to forward to a sub-agent, use the sub-agent's real name from your peer roster or azureclaw_discover.",
              });
            } else if (isSelfSend) {
            ...
            if (isRootNoParentTransfer) {
              ...
              result = JSON.stringify({
                error: "no_parent",
                message: "This agent IS the root coordinator — it has no parent to transfer files to. Resolve the real recipient name via your peer roster or azureclaw_discover.",
              });
