...
function render(inferenceBudget?: unknown, composed = false) {
  ...
  try {
    ...
    if (composed) {
      values.observationPrivacyRpc = { enabled: true };
      values.managedMcp.namespace = "customer-mcp";
    }
...
describe("governed inference budget Helm contract", () => {
  it("composes managed MCP and credential privacy with finite inference without sharing authority", () => {
    const documents = render(configured, true);
    const resource = (kind: string, name: string) => documents.find(
      (doc: { kind: string; metadata: { name: string } }) => doc.kind === kind && doc.metadata.name === name);
    const deployment = resource("Deployment", "kars-controller");
    expect(deployment.spec.replicas).toBe(3);
    const controller = deployment.spec.template.spec.containers[0];
    const envNames = controller.env.map((entry: { name: string }) => entry.name);
    expect(new Set(envNames).size).toBe(envNames.length);
    expect(controller.env).toContainEqual({ name: "MCP_MANAGED_NAMESPACE", value: "customer-mcp" });
    expect(controller.env).toContainEqual({ name: "KARS_OBSERVATION_PRIVACY_RPC_ENABLED", value: "true" });
    expect(controller.env).toContainEqual({ name: "KARS_INFERENCE_BUDGET_ADDR", value: "0.0.0.0:9447" });
    expect(controller.env).toContainEqual({ name: "CUSTOMER_SETTING", value: "preserved" });
    expect(controller.ports.map((port: { containerPort: number }) => port.containerPort))
      .toContain(9448);
    for (const [name, port] of [["kars-inference-budget", 9447], ["kars-observation-privacy", 9448]] as const) {
      const service = resource("Service", name);
      expect(service.metadata.namespace).toBe("customer-system");
      expect(service.spec.type ?? "ClusterIP").toBe("ClusterIP");
      expect(service.spec.ports[0].port).toBe(port);
      expect(service.spec.ports[0].targetPort).toBe(port);
    }
    const sandbox = resource("CustomResourceDefinition", "karssandboxes.kars.azure.com")
      .spec.versions[0].schema.openAPIV3Schema.properties.spec.properties;
    expect(sandbox).toHaveProperty("credentialBindings");
    expect(sandbox).toHaveProperty("githubBinding");
    expect(sandbox).toHaveProperty("inferenceBudgetRef");
    const material = resource("ValidatingAdmissionPolicy", "kars-observation-privacy-material");
    expect(JSON.stringify(material.spec)).toContain("dyn(namespaceObject.metadata).uid");
    expect(JSON.stringify(material.spec)).toContain("system:serviceaccount:customer-system:kars-controller");
    for (const name of ["kars-inference-budget-store", "kars-credential-source-writes", material.metadata.name]) {
