...
describe("up/resume", () => {
  ...
  it("returns null when the selected subscription differs", async () => {
    writeContext({
      phase: "network",
      subscription: "sub-a",
      region: "eastus2",
    ...
    expect(
      loadResumeState(
        {},
        { subscription: "sub-b", region: "eastus2" },
      ),
    ).toBeNull();
  });

  it("returns null when legacy partial state has no selected subscription", async () => {
    writeContext({
      phase: "network",
      region: "eastus2",
      savedAt: new Date().toISOString(),
    ...
    expect(
      loadResumeState(
        {},
        { subscription: "sub-current", region: "eastus2" },
      ),
    ).toBeNull();
  });
  ...
  it("markPhaseDone writes phase + topology to context.json and merges with prior state", async () => {
    ...
    expect(written.subscription).toBe("sub-current");       // selected subscription
