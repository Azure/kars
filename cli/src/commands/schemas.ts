...
import { renderCoreSchemaChart, schemaContextExecutor } from "../lib/core-helm-schemas.js";
import { stageCoreSchemaDocuments } from "../lib/schema-stage.js";
...
export function schemasCommand(): Command {
  ...
  command.command("prepare")
    ...
    .action(async options => {
      ...
      const { documents } = await renderCoreSchemaChart(execute, [reuse ? "upgrade" : "install", options.release, options.chart ?? requireBundledAsset("deploy/helm/kars"),
      ...
      const result = await stageCoreSchemaDocuments(execute, documents, {
