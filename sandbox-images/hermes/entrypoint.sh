...
# Inference routing: every Hermes-spawned chat call must flow
# through the kars router (loopback :8443). The router authenticates
# upstream via the controller-injected API key; the agent-visible
# value is a sentinel string so Hermes' has_usable_secret() check
# in resolve_provider() passes.
OPENAI_API_KEY=${OPENAI_API_KEY:-router-managed}
