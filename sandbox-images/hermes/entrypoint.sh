...
# Hermes 0.16 fetches the public OpenRouter catalogue during gateway startup
# even when a non-OpenRouter provider and an explicit context length are
# configured. In a governed sandbox that optional metadata lookup correctly
# hits the egress guard, but it must not create a misleading access request.
# Seed the cache with the controller-selected model so Hermes has fresh local
# metadata and never needs the unrelated external catalogue.
_HERMES_MODEL="${KARS_MODEL:-${AZURE_OPENAI_DEPLOYMENT:-gpt-5.4}}"
_HERMES_CONTEXT_LENGTH="${HERMES_MODEL_CONTEXT_LENGTH:-200000}"
case "$_HERMES_CONTEXT_LENGTH" in
  ''|*[!0-9]*) _HERMES_CONTEXT_LENGTH=200000 ;;
esac
mkdir -p "$HERMES_HOME/cache"
jq -n \
  --arg model "$_HERMES_MODEL" \
  --argjson context_length "$_HERMES_CONTEXT_LENGTH" \
  '{($model): {
      context_length: $context_length,
      max_completion_tokens: 32768,
      name: $model,
      pricing: {}
  }}' > "$HERMES_HOME/cache/openrouter_model_metadata.json.tmp"
mv "$HERMES_HOME/cache/openrouter_model_metadata.json.tmp" \
