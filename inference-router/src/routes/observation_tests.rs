...
use super::*;
use crate::{
    access_request::Identity, governed_services::GovernedServices,
    service_observation::Observer, service_observer::{Binding, Grant, Recipient},
};
...
const GRANT: &str = "/apis/kars.azure.com/v1alpha1/namespaces/workspace/karscredentialgrants/workspace";
...
fn observer_token() -> String { "o".repeat(64) }
fn control_token() -> String { "c".repeat(64) }
...
async fn fixture() -> (MockServer, AppState, Arc<Mutex<Metadata>>) {
    ...
    let binding = Binding {
        capability: CAPABILITY.into(), identity: serde_json::to_value(&identity).unwrap(),
        grant: Grant {namespace:"workspace".into(),name:"workspace".into(),uid:"grant-uid".into(),generation:1},
        recipients:vec![Recipient {namespace:"bridge".into(),namespace_uid:"bridge-uid".into(),name:"bff".into(),uid:"bff-uid".into()}],
        privacy_revision:crate::sre_privacy::REVISION.into(),privacy_epoch:None,
        server_name:"observer-sandbox-uid.kars.internal".into(),ca_pem:"-----BEGIN CERTIFICATE-----test".into(),
    };
    ...
    services.observer = Some(Observer::for_test(binding,observer_token(),"secret-uid:1".into(),client));
    let mut state = crate::routes::model_routing::tests::test_state(crate::config::Config::from_env().unwrap());
...
fn router(state:AppState) -> Router {
    Router::new()
        ...
        .layer(middleware::from_fn_with_state(state.clone(),purpose_boundary))
        .with_state(state)
}

async fn call(state: &AppState, path:&str, method:&str, token:Option<&str>, scope:Option<&str>) -> (StatusCode,Value) {
    ...
    if let Some(scope) = scope { request = request.header("x-kars-service-scope",scope); }
...
async fn observation_reads_only_sanitized_domains_with_live_metadata_and_get_list_watch_denials() {
    ...
    state.blocklist.record_learned("https://example.com/private?token=NEVER_PUBLISH").await;
    let (status,scope) = call(&state,SCOPE,"GET",Some(&observer_token()),None).await;
    assert_eq!(status,StatusCode::OK);
    let (status,body) = call(&state,LEARNED,"GET",Some(&observer_token()),scope["scope_id"].as_str()).await;
    assert_eq!(status,StatusCode::OK);
    ...
    for verb in ["get","list","watch"] {
        assert!(metadata.calls.iter().any(|(method,path,body)|method=="POST" && path==REVIEWS && body["spec"]["resourceAttributes"]["verb"]==verb));
    }
    assert!(metadata.calls.iter().all(|(method,path,_)|method=="GET" || path==REVIEWS));
    assert!(!metadata.calls.iter().any(|(_,path,_)|path.contains("/secrets")));
}
...
async fn observation_tokens_cannot_authorize_mutations_control_or_legacy_even_on_loopback() {
    let (_server,state,metadata) = fixture().await;
    for (method,path) in [
        ("POST","/internal/access-requests/reset"),("POST","/internal/access-requests/decision"),
        ("GET","/internal/access-requests"),("POST","/egress/learn"),("POST","/egress/learned/clear"),
        ("GET","/egress/learned"),("POST",LEARNED),
    ] {
        assert_eq!(call(&state,path,method,Some(&observer_token()),None).await.0,StatusCode::FORBIDDEN,"{method} {path}");
    }
    for token in [None,Some("legacy-agent-token".into()),Some(control_token())] {
        assert_eq!(call(&state,SCOPE,"GET",token.as_deref(),None).await.0,StatusCode::FORBIDDEN);
    }
...
async fn observation_rejects_replaced_foreign_or_revoked_authority_and_stale_rollout() {
    for (path,pointer,replacement) in [
        ...
        (SANDBOX,"/status/serviceObservation/version",json!("secret-uid:2")),
        (SANDBOX,"/status/serviceObservation/phase",json!("Prepared")),
        (SANDBOX,"/status/serviceObservation/grant/uid",json!("foreign")),
        ...
        ("/api/v1/namespaces/kars-agent","/metadata/uid",json!("replacement")),
        ("/api/v1/namespaces/bridge","/metadata/uid",json!("replacement")),
        (RECIPIENT,"/metadata/uid",json!("replacement")),
    ] {
        let (_server,state,metadata) = fixture().await;
        *metadata.lock().unwrap().objects.get_mut(path).unwrap().pointer_mut(pointer).unwrap() = replacement;
        assert_eq!(call(&state,SCOPE,"GET",Some(&observer_token()),None).await.0,StatusCode::FORBIDDEN,"{path}{pointer}");
    }
}
...
async fn observation_privacy_pending_null_ready_or_authorized_legacy_subject_fails_closed() {
    for phase in ["Migrating","Pending","Ready"] {
        ...
        assert_eq!(call(&state,SCOPE,"GET",Some(&observer_token()),None).await.0,StatusCode::FORBIDDEN,"{phase}");
    }
    for verb in ["get","list","watch"] {
        let (_server,state,metadata) = fixture().await;
        metadata.lock().unwrap().allow = Some(verb.into());
        assert_eq!(call(&state,SCOPE,"GET",Some(&observer_token()),None).await.0,StatusCode::FORBIDDEN,"{verb}");
    }
    let (_server,state,metadata) = fixture().await;
    metadata.lock().unwrap().fail=Some(GRANT.into());
    assert_eq!(call(&state,SCOPE,"GET",Some(&observer_token()),None).await.0,StatusCode::FORBIDDEN);
}
...
async fn observation_scope_resets_are_cas_fenced_and_missing_capability_is_unavailable() {
    ...
    assert_eq!(call(&state,LEARNED,"GET",Some(&observer_token()),Some(&current.id)).await.0,StatusCode::CONFLICT);
    let fresh = state.services.requests.scope().unwrap();
    assert_eq!(call(&state,LEARNED,"GET",Some(&observer_token()),Some(&fresh.id)).await.0,StatusCode::OK);
    Arc::get_mut(&mut state.services).unwrap().observer=None;
    assert_eq!(call(&state,SCOPE,"GET",Some(&observer_token()),None).await.0,StatusCode::SERVICE_UNAVAILABLE);
}
