...
fn fixture() -> (GitHubBinding, KarsCredentialGrant, ConfigMap, Secret, Value) {
    let name = format!(
        ...
        &crate::providers::signing::sha256_hex(b"owner-subject")[..16]
    );
    let selection = GitHubBinding {
    ...
    let key = rcgen::KeyPair::generate_for(&rcgen::PKCS_RSA_SHA256)
        .unwrap()
        .serialize_pem();
...
fn governed_github_factory_canonicalizes_app_id_without_mutating_the_customer_store() {
    ...
    store.data.as_mut().unwrap().insert(
        "GITHUB_APP_ID".into(),
        k8s_openapi::ByteString(b"00123".to_vec()),
    );
    let value: Value = serde_json::from_str(
        &configuration(&selection, &grant, &connection, &store, &identity).unwrap(),
    )
