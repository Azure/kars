...
fn string(secret:&Secret,key:&str)->Result<String,String> {
    secret.data.as_ref().and_then(|data|data.get(key))
        .and_then(|data|std::str::from_utf8(&data.0).ok()).map(str::to_string)
...
fn configuration(
    ...
    let expected_name=format!("kars-github-connection-{}",hex::encode(&Sha256::digest(approved.owner_subject.as_bytes())[..8]));
    if approved.owner_subject.is_empty() || expected_name!=connection.name_any()
    ...
    let data=connection.data.as_ref().ok_or("GitHub connection metadata is unavailable")?;
    let installation=data.get("installation_id").and_then(|id|id.parse::<u64>().ok());
    let repositories:Vec<String>=serde_json::from_str(data.get("repos").ok_or("GitHub connection repositories missing")?)
        .map_err(|_|"GitHub connection repositories are invalid")?;
    if installation!=Some(approved.installation_id)
        || selection.repositories.iter().any(|repo|!repositories.iter().any(|actual|actual.to_ascii_lowercase()==*repo))
    {
    ...
    if app!=approved.app_id || app.is_empty() || app.len()>20 || !app.bytes().all(|byte|byte.is_ascii_digit())
    ...
    if serialized.len()>65536 {return Err("GitHub private configuration exceeds the consumer limit".into())}
    Ok(serialized)
}

async fn prepare(
    client:&Client,sandbox:&KarsSandbox,managed_identity:&Value,
) -> Result<(KarsCredentialGrant,ConfigMap,Secret,String),String> {
    let selection=sandbox.spec.github_binding.as_ref().ok_or("GitHub selection missing")?;
    ...
    if sandbox.spec.credentials_ref.is_some()
        || sandbox.spec.network_policy.as_ref().is_none_or(|policy|
            !policy.default_deny || policy.egress_mode!=crate::crd::EgressMode::Strict || policy.allowlist_ref.is_some()
            || policy.allowed_endpoints.iter().flatten().any(|endpoint|contract::opaque_github_egress(&endpoint.host)))
    {
    ...
    if let Some(task_uid)=managed_identity["task"]["uid"].as_str() {
        ...
        let task=Api::<crate::kars_task::KarsTask>::namespaced(client.clone(),&workspace).get(name).await
            .map_err(|e|api_error("Read GitHub Task authorization",e))?;
        if task.uid().as_deref()!=Some(task_uid) || !crate::kars_task_reconciler::task_is_ready(&task)
            || task.spec.blueprint.as_ref().and_then(|blueprint|blueprint.github_binding.as_ref())!=Some(selection)
...
pub(super) async fn revoke(client:&Client,grant:&KarsCredentialGrant)->Result<(),String> {
    ...
    for sandbox in Api::<KarsSandbox>::namespaced(client.clone(),&workspace).list(&ListParams::default()).await
        .map_err(|e|api_error("Read enrolled GitHub consumers",e))?
    {
        if sandbox.metadata.annotations.as_ref().and_then(|values|values.get(ENROLLED))==grant.metadata.uid.as_ref() {
            let namespace=Api::<Namespace>::all(client.clone()).get(&format!("kars-{}",sandbox.name_any())).await
...
async fn issue(
    ...
    let current_sandbox=sandboxes.get(&sandbox.name_any()).await.map_err(|e|api_error("Refresh GitHub target",e))?;
    if current_sandbox.uid()!=sandbox.uid() || current_sandbox.spec.github_binding!=sandbox.spec.github_binding
        || current_sandbox.metadata.generation!=sandbox.metadata.generation || current_sandbox.metadata.deletion_timestamp.is_some()
    {return Err("GitHub target changed before private issuance".into())}
    if current_sandbox.metadata.annotations.as_ref().and_then(|values|values.get(ENROLLED))!=grant.metadata.uid.as_ref() {
    ...
    let live_connection=Api::<ConfigMap>::namespaced(client.clone(),&workspace).get_metadata(&connection.name_any()).await
        .map_err(|e|api_error("Recheck GitHub connection identity",e))?;
    let live_store=Api::<Secret>::namespaced(client.clone(),&workspace).get_metadata(&store.name_any()).await
