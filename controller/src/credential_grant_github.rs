...
pub fn repository(value: &str) -> bool {
    ...
    let part = |part: &str, max: usize| !part.is_empty() && part.len() <= max
        && ![".", ".."].contains(&part)
        && part.bytes().all(|byte| byte.is_ascii_lowercase() || byte.is_ascii_digit() || b"._-".contains(&byte));
    part(owner,39) && part(repo,100)
}

pub fn validate(binding: &GitHubBinding) -> Result<(),String> {
    if binding.grant.name != NAME || binding.grant.uid.is_empty()
        || !binding.connection.name.starts_with("kars-github-connection-") || binding.connection.uid.is_empty()
        || binding.repositories.is_empty() || binding.repositories.len()>32
        || binding.repositories.iter().any(|repo| !repository(repo))
        || binding.repositories.iter().collect::<std::collections::BTreeSet<_>>().len()!=binding.repositories.len()
    {
...
pub fn attenuates(child:Option<&GitHubBinding>,parent:Option<&GitHubBinding>) -> bool {
    ...
    child.grant==parent.grant && child.connection==parent.connection
        && (!child.write || parent.write)
        && child.repositories.iter().all(|repo|parent.repositories.contains(repo))
}
...
pub fn agent_sources(bindings:Option<&CredentialBindings>) -> Result<(),String> {
    ...
    if bindings.sources.iter().flat_map(|source|&source.keys)
...
pub fn opaque_github_egress(host:&str) -> bool {
    ...
    host=="*" || ["github.com","api.github.com"].iter().any(|target|
        host==*target || host.strip_prefix("*.").is_some_and(|suffix|*target==suffix || target.ends_with(&format!(".{suffix}"))))
}
...
mod tests {
    ...
    fn binding()->GitHubBinding {
        GitHubBinding {grant:ObjectIdentity{name:NAME.into(),uid:"grant".into()},
            connection:ObjectIdentity{name:"kars-github-connection-test".into(),uid:"connection".into()},
            repositories:vec!["owner/repo".into()],write:false}
    }
    #[test]
    fn governed_github_bindings_reject_alias_paths_and_attenuate_repositories_write_and_uids(){
        ...
        for repo in ["Owner/repo","owner/../repo","owner/repo.git/extra","owner/%2e","owner/..","owner/"] {
            let mut child=parent.clone();child.repositories=vec![repo.into()];
    ...
    fn governed_github_rejects_opaque_api_egress_and_implicit_legacy_credentials(){
        for host in ["github.com","api.github.com","*.github.com","*.com","*","GITHUB.COM."] {
    ...
    fn governed_github_selection_is_part_of_the_existing_full_task_authorization_digest(){
        let model=crate::kars_task::TaskModel{provider:"test".into(),deployment:"test".into()};
        let mut task=crate::kars_task::KarsTaskSpec{
            blueprint:Some(crate::kars_task::TaskBlueprint{github_binding:Some(binding()),..Default::default()}),
            ..Default::default()
        };
        let original=task.authorization_digest_with_model(&model);
        assert_eq!(task.authorization_configuration_with_model(&model)["blueprint"]["githubBinding"]["connection"]["uid"],"connection");
        task.blueprint.as_mut().unwrap().github_binding.as_mut().unwrap().connection.uid="replacement".into();
